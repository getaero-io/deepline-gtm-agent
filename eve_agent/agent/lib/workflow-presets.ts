...
export const WORKFLOW_PRESETS: Record<string, WorkflowPreset> = {
  inbound_lead_approval: {
    ...
    prompt:
      "Research this inbound lead, decide whether we should act, draft the next best outreach, and ask for rep approval before sending or CRM writeback. Show sources, reasons not to act, and missing context.",
  ...
  account_digest: {
    ...
    prompt:
      "Create a weekly account digest for this territory. Rank accounts by what changed, source every signal, and return the top 3 actions. Do not write back without approval.",
    suggested_tool_bounds: {
      maxToolCalls: 12,
      side_effects_require_approval: true,
    },
  ...
  self_serve_support_agent: {
    ...
    prompt:
      "Answer this onboarding/support question using current docs and known policy context. Stream progress, cite the source, and escalate if the answer touches legal, pricing exceptions, or live pairing.",
    suggested_tool_bounds: {
      maxToolCalls: 6,
      side_effects_require_approval: true,
    },
  ...
  web_context_research: {
    ...
    prompt:
      "Research this account from web sources. Return source-backed claims, freshness, relevance to GTM, confidence, missing context, and the next safe workflow. Do not enrich contacts or write to CRM.",
    suggested_tool_bounds: {
  ...
  bounded_tool_action: {
    ...
    prompt:
      "Before using any tool, state the tool, why it is needed, whether it creates a side effect, and whether approval is required. Use the minimum tool set and return an audit trail.",
    suggested_tool_bounds: {
      maxToolCalls: 5,
      side_effects_require_approval: true,
    },
  ...
  closed_loop_gtm_workflow: {
    ...
    prompt:
      "Combine first-party and third-party context, recommend the next GTM action, ask for approval, and after approval write back the result and the learning signal. Report provider misses and marginal cost signals.",
    suggested_tool_bounds: {
      maxToolCalls: 10,
      side_effects_require_approval: true,
    },
  ...
  snowflake_query_agent: {
    ...
    prompt:
      "Answer this GTM question using Snowflake/warehouse context. First identify the likely tables and fields, then propose the SQL before running it. Use read-only SELECT queries only. Limit exploratory queries, explain joins and filters, return the result with source tables, and ask for approval before any writeback or downstream action.",
    suggested_tool_bounds: {
      enabledToolIds: [
        "deeplineagent",
        "snowflake_query",
